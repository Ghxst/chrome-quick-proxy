document.addEventListener("DOMContentLoaded", () => {
    ...
    const harBtn = $("harBtn"), harFile = $("harFile"), harCtl = $("harControls"),
        entrySel = $("entrySelect"), sideSel = $("sideSelect"), harImp = $("importCookies");
    ...
    const bg = (m, cb) => chrome.runtime.sendMessage(m, cb);
    const setC = (o) => new Promise((r, j) => chrome.cookies.set(o, () => chrome.runtime.lastError ? j(chrome.runtime.lastError) : r()));
    const urlF = (c, d) => (c.secure ? "https://" : "http://") + (c.domain || new URL(d).hostname).replace(/^\./, "") + (c.path || "/");
    ...
    harFile.addEventListener("change", () => {
        ...
        rd.onload = e => {
            ...
            } catch { alert("HAR error"); harEntries = []; harCtl.style.display = "none"; }
        };
    ...
    harImp.addEventListener("click", async () => {
        ...
        let ok = 0;
        for (const c of jar) try {
            await setC({ url: urlF(c, en.request.url), name: c.name, value: c.value, domain: c.domain, path: c.path || "/", secure: !!c.secure, httpOnly: !!c.httpOnly, expirationDate: c.expires ? Number(c.expires) : undefined });
            ok++;
        } catch { }
    ...
    sessBtn.addEventListener("click", async () => {
        ...
        let d; try { d = JSON.parse(txt); } catch { return alert("Invalid JSON"); }
        const jar = d.cookies || []; if (!jar.length) return alert("No cookies field.");
        let ok = 0;
        for (const c of jar) if (c.name && c.value && c.domain) try {
            await setC({ url: urlF(c, "https://" + c.domain.replace(/^\./, "")), name: c.name, value: c.value, domain: c.domain, path: c.path || "/", secure: !!c.secure, httpOnly: !!c.httpOnly, expirationDate: c.expires ? Number(c.expires) : undefined });
            ok++;
        } catch { }
    ...
    exportBtn.addEventListener("click", () => {
        if (!confirm("Warning: this will export ALL cookies stored in Chrome, not just for the current site. Continue?")) return;
    ...
    importFile.addEventListener("change", () => {
        ...
        r.onload = async e => {
            let jar; try { jar = JSON.parse(e.target.result); } catch { return alert("Invalid JSON"); }
            if (!Array.isArray(jar) || !jar.length) return alert("No cookies array.");
            let ok = 0;
            for (const c of jar) if (c.name && c.value && c.domain) try {
                await setC({ url: urlF(c, "https://" + c.domain.replace(/^\./, "")), name: c.name, value: c.value, domain: c.domain, path: c.path || "/", secure: !!c.secure, httpOnly: !!c.httpOnly, expirationDate: c.expirationDate ?? (c.expires ? Number(c.expires) : undefined) });
                ok++;
            } catch { }
            alert(`Imported ${ok}/${jar.length} cookies`);
        };
